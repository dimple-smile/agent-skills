...
1. 寓言作为**末章**插入已生成页面：放在「记」（takeaway）区块**之后**、
   `</div><!-- /container -->` 之前 —— 全页最后一个 section。
...
3. 标题层级：本节用 `<h2>`（与第 2-6 层同级），不再降级 —— 11 门的
   heading-monotonic 不会因此触发。
4. 完成后重跑 `aha check`，回执沿用 Step 5 格式（`修复轮次` 字段照常），
   并附一句寓言标题（不剧透内容）。
