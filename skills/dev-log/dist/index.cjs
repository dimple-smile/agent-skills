...
var LOG_FILE = import_path.default.join(process.cwd(), "dev-logs.json");
var PID_FILE = import_path.default.join(process.cwd(), "pid.txt");
var PORT_FILE = import_path.default.join(process.cwd(), "port.txt");
var TUNNEL_URL_FILE = import_path.default.join(process.cwd(), "tunnel-url.txt");
var MAX_BODY_SIZE = 10 * 1024 * 1024 * 1024;
var addresses = {
  local: null,
  network: null,
  tunnel: null
...
function readLogs(sessionId) {
  try {
    const data = import_fs.default.readFileSync(LOG_FILE, "utf-8");
...
function handlePostLogs(req, res) {
  ...
  req.on("end", () => {
    ...
    try {
      ...
      import_fs.default.writeFileSync(LOG_FILE, JSON.stringify([...existingLogs, ...newLogs], null, 2));
...
function killOldProcess() {
  if (import_fs.default.existsSync(PID_FILE)) {
    const pid = parseInt(import_fs.default.readFileSync(PID_FILE, "utf-8").trim(), 10);
    if (pid && !isNaN(pid)) {
      try {
        process.kill(pid, 0);
...
function handleRequest(req, res) {
  ...
  const urlObj = new import_url.URL(fullUrl, `http://localhost:${addresses.local || 3e3}`);
...
function createServer() {
  return import_http.default.createServer(handleRequest);
}
async function startTunnel(port) {
  try {
    console.log("Starting tunnel...");
    const tunnel = await (0, import_localtunnel.default)({ port });
    console.log(`Tunnel started: ${tunnel.url}`);
    import_fs.default.writeFileSync(TUNNEL_URL_FILE, tunnel.url);
    addresses.tunnel = tunnel.url;
    tunnel.on("close", () => {
      console.log("Tunnel closed");
      addresses.tunnel = null;
      if (import_fs.default.existsSync(TUNNEL_URL_FILE)) {
        import_fs.default.unlinkSync(TUNNEL_URL_FILE);
      }
    });
    return tunnel.url;
  } catch (e) {
...
async function startServer() {
  ...
  [PORT_FILE, TUNNEL_URL_FILE].forEach((f) => {
    if (import_fs.default.existsSync(f)) import_fs.default.unlinkSync(f);
  });
  ...
  await new Promise((resolve) => {
    httpServer.listen(0, async () => {
      ...
      import_fs.default.writeFileSync(PID_FILE, String(pid));
      import_fs.default.writeFileSync(PORT_FILE, String(httpPort2));
